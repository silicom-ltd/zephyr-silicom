...
#if CONFIG_DT_HAS_MICROCHIP_XEC_ESPI_MAF_FLASH_ENABLED
#include <zephyr/drivers/flash.h>

#if DT_PROP(DT_INST(0, microchip_xec_maf_flash), max_sectors)
#define FLASH_XEC_MAF_MAX_SECTORS \
	DT_PROP(DT_INST(0, microchip_xec_maf_flash), max_sectors)
#else
#error Flash MAF max sectors not available
	/* Flash MAF max sectors is extracted from device tree */
	/* as flash node property 'max-sectors' */
#endif
...
static const struct flash_pages_layout espi_flash_layout[] = {
	{
	.pages_size = 4096,
	.pages_count = FLASH_XEC_MAF_MAX_SECTORS,
	}
};
#endif
